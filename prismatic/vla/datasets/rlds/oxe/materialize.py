...
def make_oxe_action_dataset_kwargs(
    dataset_name: str,
    data_root_dir: Path,
    load_camera_views: Tuple[str] = None,
    load_depth: bool = False,
    load_proprio: bool = True,
    load_language: bool = True,
    action_proprio_normalization_type: NormalizationType = NormalizationType.NORMAL,
    ...
    if load_camera_views:
        ...
        # Remove image and depth observations if no camera views are requested
        dataset_kwargs.pop("image_obs_keys", None)
...
def get_oxe_dataset_kwargs_and_weights(
    ...
    :param load_camera_views: Camera views to load; see `oxe.dataset_configs.py` for available views. If None, no camera views are loaded.
    ...
    for d_name, d_weight in filtered_mixture_spec:
        try:
            per_dataset_kwargs.append(
                make_oxe_action_dataset_kwargs(
                    d_name,
                    data_root_dir,
                    load_camera_views,
                    load_depth,
